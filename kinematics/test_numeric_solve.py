...
def test_plane_xonly_chain():
    ...
    chain.append('link2', [2, 0, 0], lambda q: [0, 0, 0])

    assert_allclose(
        numeric_solve(chain, transform([3, 0, 0], [0, 0, 0]))[1][:2],
        [0, 0], atol=atol)

    assert_allclose(
        numeric_solve(chain, transform([0, 3, 0], [0, 0, pi / 2]))[1][:2],
        [pi / 2, 0], atol=atol)
